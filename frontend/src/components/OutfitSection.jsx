...
const SkeletonOutfitCard = () => (
  <GlassCard className="flex flex-col h-full w-full p-6 border-2 border-white/5 animate-pulse">
...
export function OutfitSection({ result, outfits, onRefresh, onCurate, loading }) {
  const [event, setEvent] = React.useState('');
  const [season, setSeason] = React.useState('Summer');

  if ((!outfits || outfits.length === 0) && !loading) return null;
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 md:mb-12">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 bg-white/5 p-2 rounded-2xl border border-white/5 backdrop-blur-xl">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4">
             <div className="flex items-center gap-2 px-4 py-2 bg-zinc-900/50 rounded-xl border border-white/5 justify-between sm:justify-start w-full sm:w-auto min-w-[180px]">
                <span className="text-[10px] font-black text-zinc-500 uppercase">Event</span>
                <select 
                  value={event}
                  onChange={(e) => setEvent(e.target.value)}
                  className="bg-transparent text-sm font-bold text-white focus:outline-none cursor-pointer"
                >
                  <option value="" className="bg-zinc-900 text-white">Casual</option>
                  <option value="Work" className="bg-zinc-900 text-white">Office / Formal</option>
                  <option value="Night" className="bg-zinc-900 text-white">Night Out / Party</option>
                  <option value="Wedding" className="bg-zinc-900 text-white">Wedding / Gala</option>
                  <option value="Street" className="bg-zinc-900 text-white">Active / Street</option>
                </select>
             </div>
             <div className="flex items-center gap-2">
               <button 
                 onClick={() => onCurate(event, season)}
                 className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-8 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 rounded-xl text-sm font-black text-white uppercase tracking-widest hover:scale-105 transition-transform shadow-lg shadow-purple-500/20 active:scale-95"
               >
                 {event === (result.event || "") && season === (result.season || "Summer") ? (
